...
| 種類 | 拡張子 | 備考 |
| --- | --- | --- |
| WAV | `.wav` | マルチチャンネル WAV と、一時的な raw PCM 読み直しに対応します。 |
| エンコード音声 | `.mp3`、`.flac`、`.ogg`、`.opus`、`.m4a`、`.aac` | まず VS Code Webview のデコーダを使います。必要に応じて、拡張ホスト側の FFmpeg によるフォールバック変換を使います。 |
...
リモート音声をローカルに保存したい場合は、上部バーのダウンロードボタンを使えます。
