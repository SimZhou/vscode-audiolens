...
| マルチチャンネル音声 | Audacity 風のチャンネルトラック、チャンネルごとの波形/スペクトログラム、ミュート、ソロ、チャンネルごとのゲイン/パン再生。ステレオ音声は既定でチャンネル 1 を左、チャンネル 2 を右いっぱいに振り、モノラルと 3 チャンネル以上は中央にダウンミックスします。チャンネルごとに周波数スケール/範囲と振幅を調整でき、縦軸の目盛りはトラック高さに追従します。 |
...
| スペクトログラム解析 | 周波数領域 STFT 解析、最大 `32768` の FFT、多様な窓関数、周波数スケール、パレット、自動輝度。 |
...
| 設定保存 | デフォルトのトラック表示、スペクトログラム設定、PCM 既定値、表示言語を保存します。 |
...
タイムラインの黄色いラベルには再生位置が小数点以下 3 桁で表示され、ドラッグして移動できます。選択範囲がある場合は、その範囲を保ったまま範囲内で移動します。再生中のドラッグでは一時停止し、離すと新しい位置から再開しますが、範囲の終端では停止したままになります。
...
- ゼロパディング倍率: `1` から `128`（メモリ使用量を制限するため padded FFT は最大 `131072`）
