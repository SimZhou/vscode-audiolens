...
| マルチチャンネル音声 | Audacity 風のチャンネルトラック、チャンネルごとの波形/スペクトログラム、ミュート、ソロ、チャンネルごとのゲイン/パン再生（既定は downmix、左右いっぱいに振ればステレオそのまま）。チャンネルごとに周波数スケール/範囲と振幅を調整でき、縦軸の目盛りはトラック高さに追従します。 |
