...
| Multi-channel audio | Audacity-style channel tracks, per-channel waveform/spectrogram views, mute, solo, and downmix or bypass playback. |
...
| Area | Features |
| --- | --- |
| Playback | Keyboard-ready `Space` play/pause, seek, selection playback, playback gain, downmix/bypass mode, per-channel mute and solo. |
...
| Persistence | Saves default track view, spectrogram settings, playback gain, playback mode, PCM defaults, and language preference. |
...
| Switch decoded audio playback mode | Transport `Mode` selector (`Downmix` / `Bypass`) |
