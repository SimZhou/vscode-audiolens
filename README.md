...
| Multi-channel audio | Audacity-style channel tracks, per-channel waveform/spectrogram views, mute, solo, and per-channel gain/pan playback (downmix by default, full stereo when panned hard left/right). Per-channel frequency scale/range and amplitude, with axis ticks that adapt to track height. |
